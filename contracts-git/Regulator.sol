...
*/
   ...
   function createConsentRequest(string memory id,uint16 company_id,uint16 kyc_manager_id) public   {
    ...
    event finalizeKYC(string    id,address KYCInt,uint16 company_registry_id ,uint timestamp);

    function performFullKYC( string memory id,uint16 _kycPerformer_registry_id, string[] memory   attributesList, string[] memory   attributesVal) public
     {

            require(check_company_registry_id_against_address(_kycPerformer_registry_id),"this company not allowed to make setting!Wrong address!");

            bytes32 idbytes32=id.stringToBytes32();
            RegulatorLib.Consumer storage consumer=self.consumers[idbytes32];
            //KYCInt  kyc = new KYC( address(this) , id,_kycPerformer_registry_id,attributesList,    attributesVal);
            address kycaddr=KYCFactory.createKyc( id,_kycPerformer_registry_id,attributesList,    attributesVal);
            //address kycaddr=address(kyc);
           // consumer.kyc =  address(kyc);
            consumer.kyc = kycaddr;
            consumer.verified=true;
            emit finalizeKYC(id,kycaddr, _kycPerformer_registry_id ,block.timestamp);

            addCompanionPermission(id , _kycPerformer_registry_id,"*",1);
    ...
    function getConsumerAttributeList( string memory id,uint16 company_registry_id) public view returns(bytes32[] memory)
             {
                ...
                require(self.consumers[idbytes32].kyc != address(0)  ,"KYC not exist for cusstomer");
                 ...
                 return KYCInt(self.consumers[idbytes32].kyc).getAttributeList();
    ...
    function addCompanionPermission(string memory id , uint16 company_registry_id,string memory attributeName,uint8   attributepermission) public   {
             ...
             //require(KYCInt(permissionExtenderAddress).getCustomerAddress ==getConsumerAddress(id));
            ...
            require(check_company_registry_id_against_address(getCurrentKYCPIssuer(id)),"this company not allowed to make setting!Wrong address!");
             ...
             KYCInt(self.consumers[idbytes32].kyc).setAttributePermission(attributeName,company_registry_id,attributepermission);
     ...
     function getConsumerAttributePermission( string memory id,uint16 company_registry_id,string memory attributeName) public view returns(uint8 permission)
         {
            ...
            require(self.consumers[idbytes32].kyc != address(0)  ,"KYC not exist for cusstomer");
             ...
             return KYCInt(self.consumers[idbytes32].kyc).isAttributePermited(attributeName,company_registry_id);
...
/*
        ...
        */
          function getConsumerAttributeValue(string memory id,uint16 company_registry_id,string memory attributeName) public view returns(bytes32 )
              {
               ...
               return self.getConsumerAttributeValue(id, company_registry_id, attributeName);
               }
         ...
         function getConsumerAttributeName(string memory id,uint row) public view returns(bytes32 )
         {
            ...
            return KYCInt(self.consumers[idbytes32].kyc).getAttributeName( row);
         ...
         function getAttributeLength(string memory id) public view returns(uint )
         {
            ...
            bytes32 idbytes32=id.stringToBytes32();
            require(self.consumers[idbytes32].kyc!=address(0),"KYC not exist for cusstomer");
            return KYCInt(self.consumers[idbytes32].kyc).getAttributeLength( );
    ...
    function getCurrentKYCPIssuer(string memory id) public view returns(uint16 )
    {
                ...
                return KYCInt(self.consumers[idbytes32].kyc).getKYCIssuer( );
